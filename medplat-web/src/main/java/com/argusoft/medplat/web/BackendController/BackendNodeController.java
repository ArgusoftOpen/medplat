...
import com.argusoft.medplat.web.BackendController.repository.IndicatorMasterRepository;
import com.argusoft.medplat.web.BackendController.repository.DerivedAttributesRepository;
import com.argusoft.medplat.web.BackendController.repository.DatasetMasterRepository;
import com.argusoft.medplat.web.BackendController.repository.GenericRepository;
...
public class BackendNodeController {
    ...
    @Autowired
    private IndicatorMasterRepository indicatorMasterRepository;

    @Autowired
    private DerivedAttributesRepository derivedAttributesRepository;
    ...
    public Map<String, Object> getAttributeValue(@RequestBody Map<String, Object> body) {
        ...
        if (indicator != null) {
            Integer value = indicatorMasterRepository.getQueryResultByIndicatorName(indicator);
            return Map.of("value", value != null ? value : 0);
        } else if (table != null && column != null) {
            Integer value = genericRepository.getColumnCount(table, column);
            return Map.of("value", value);
        } else {
            throw new IllegalArgumentException("Missing table/column or indicator");
        }
    ...
    @GetMapping("/tables")
    public List<String> getAllTables() {
        return genericRepository.getAllTables();
    }

    // GET /api/columns/:table
    @GetMapping("/columns/{table}")
    public List<String> getColumns(@PathVariable String table) {
        return genericRepository.getColumns(table);
    }

    // GET /api/table-metadata/:table
    @GetMapping("/table-metadata/{table}")
    public List<Map<String, Object>> getTableMetadata(@PathVariable String table) {
        return genericRepository.getTableMetadata(table);
    }

    // GET /api/sample-data/:table?limit=10
    @GetMapping("/sample-data/{table}")
    public List<Map<String, Object>> getSampleData(@PathVariable String table, @RequestParam(defaultValue = "10") int limit) {
        return genericRepository.getSampleData(table, limit);
    }
    ...
    public List<Map<String, Object>> getData(@RequestBody Map<String, Object> body) {
        ...
        return genericRepository.getData(table, columns);
    }
    ...
    public Map<String, Object> previewSql(@RequestBody Map<String, Object> body) {
        ...
        List<Map<String, Object>> rows = genericRepository.executeQuery(previewSql);
        return Map.of("rows", rows);
    }
    ...
    public Map<String, Object> saveDerivedAttribute(@RequestBody Map<String, Object> body) {
        ...
        DerivedAttributes derivedAttribute = new DerivedAttributes();
        derivedAttribute.setDerivedName(derivedName);
        derivedAttribute.setFormula(formula);
        derivedAttribute.setResult(resultVal);

        derivedAttributesRepository.save(derivedAttribute);
        return Map.of("success", true);
    }

    // GET /api/derived-attributes
    @GetMapping("/derived-attributes")
    public List<DerivedAttributes> getDerivedAttributes() {
        return derivedAttributesRepository.findAllByOrderByCreatedOnDesc();
    }
    ...
    public Map<String, Object> saveIndicatorMaster(@RequestBody Map<String, Object> body) {
        ...
        List<Map<String, Object>> rows = genericRepository.executeQuery(sqlQuery);
        Integer queryResult = null;
        if (!rows.isEmpty()) {
            Map<String, Object> firstRow = rows.get(0);
            for (Object v : firstRow.values()) {
        ...
        if (createdBy instanceof Number) {
            indicatorMaster.setCreatedBy(((Number) createdBy).intValue());
        }

        indicatorMasterRepository.save(indicatorMaster);
        return Map.of("success", true, "query_result", queryResult);
    }

    // GET /api/indicator-master
    @GetMapping("/indicator-master")
    public List<IndicatorMaster> getIndicatorMaster() {
        return indicatorMasterRepository.findAll();
    }
    ...
    public List<Map<String, Object>> getGroupedCount(@RequestBody Map<String, Object> body) {
        ...
        return genericRepository.getGroupedCount(table, xAxis, indicator);
    }
    ...
    public Map<String, Object> runSql(@RequestBody Map<String, Object> body) {
        String query = (String) body.get("query");
        if (query == null) throw new IllegalArgumentException("Missing SQL query");
        List<Map<String, Object>> rows = genericRepository.executeQuery(query);
        List<String> fields = rows.isEmpty() ? new ArrayList<>() : new ArrayList<>(rows.get(0).keySet());
        return Map.of("rows", rows, "fields", fields);
    }
    ...
    // GET /api/dataset-master/:id
    @GetMapping("/dataset-master/{id}")
    public Optional<DatasetMaster> getDatasetMasterById(@PathVariable Integer id) {
        return datasetMasterRepository.findById(id);
    }

    // GET /api/dataset-master
    @GetMapping("/dataset-master")
    public List<DatasetMaster> getDatasetMaster() {
        return datasetMasterRepository.findAllByOrderByIdDesc();
    }
    ...
    public Map<String, Object> saveDataset(@RequestBody Map<String, Object> body) {
        ...
        datasetMasterRepository.save(datasetMaster);
        return Map.of("success", true, "message", String.format("Dataset '%s' created.", tableName));
    }
    ...
    public List<Map<String, Object>> getDatasetChart(@RequestBody Map<String, Object> body) {
        String sql = (String) body.get("sql");
        if (sql == null) throw new IllegalArgumentException("Invalid SQL query");
        List<Map<String, Object>> rows = genericRepository.executeQuery(sql);
        if (rows.isEmpty()) return new ArrayList<>();
