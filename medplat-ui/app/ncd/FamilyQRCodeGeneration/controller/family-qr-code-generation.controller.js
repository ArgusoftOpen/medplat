(function (angular) {
    function FamilyQRCodeGenerationController(FamilyQRCodeDAO, AuthenticateService, PagingService, $scope, Mask, GeneralUtil, $rootScope) {
        const ctrl = this;
        $scope.ctrl = ctrl;
        ctrl.pagingService = PagingService.initialize();
        ctrl.requiredLevel = 5;
        ...
        ctrl.getFamilyList = function (reset) {
            ...
            if (ctrl.selectedLocationId) {
                ...
                PagingService.getNextPage(FamilyQRCodeDAO.getFamilies, dto, familyList, null).then(function (res) {
                    ctrl.familyList = res;
                }).catch((error) => {
                    GeneralUtil.showMessageOnApiCallFailure(error);
                }).finally(() => {
        ...
        ctrl.printAllFamily = function (familyId) {
            ...
            if (familyId == -1) {
                ...
                FamilyQRCodeDAO.generatePdf(dto).then(res => {
    ...
    angular.module('imtecho.controllers').controller('FamilyQRCodeGenerationController', FamilyQRCodeGenerationController);
})(window.angular);