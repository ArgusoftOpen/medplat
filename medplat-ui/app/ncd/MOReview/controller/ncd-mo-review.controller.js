(function (angular) {
    function NcdMOReviewController(PagingService, NcdDAO, Mask, $filter, $state, $window, GeneralUtil) {
        var ctrl = this;
        ...
        var init = function () {
            ...
            ctrl.retrieveMemberList('moReview', true);
        ...
        ctrl.searchMembers = function (reset) {
            ...
            if (ctrl.searchForm.$valid) {
                ...
                if(criteria.searchBy === 'orgUnit'){
                    if (ctrl.search.orgUnitType) {
                        criteria.searchString = criteria.searchString + "#" + ctrl.search.orgUnitType
                    }
                    else {
                ...
                if(criteria.searchBy === 'name'){
                    criteria.searchString = criteria.searchString  + "#" + ctrl.search.firstName + "#" + ctrl.search.lastName;
                ...
                //call backend api
                ctrl.membersdemo = [];
                PagingService.getNextPage(NcdDAO.retreiveSearchedMembersMOReview, criteria, ctrl.membersSearchList, null).then(function (res) {
        ...
        ctrl.downloadExcel = () => {
            ...
            if (ctrl.isSearch) {
                if (ctrl.searchForm.$valid) {
                    ...
                    //call backend api
                    NcdDAO.retreiveSearchedMembersMOReview(criteria).then(function (response) {
                        if (response != null && response.length > 0) {
                            response.forEach((member) => {
                                ...
                                ctrl.excelMembers.push({
                                    ...
                                    "Type": member.subStatus!='CVC'?$filter('titlecase')(member.subStatus):member.subStatus,
        ...
        ctrl.fillExamineForm = function (id) {
            let url = $state.href('techo.ncd.moreviewmember', { id: id});
    ...
    angular.module('imtecho.controllers').controller('NcdMOReviewController', NcdMOReviewController);
})(window.angular);